...
输出路径策略：
- 强制优先：`~/clawdhome_shared/public/<skill-slug>-data/`（例如 `~/clawdhome_shared/public/meeting-notes-skill-data/`）。
- 若 `public` 不可写，再回退到 `~/clawdhome_shared/vault/<skill-slug>-data/`。
- 若 `public` 和 `vault` 都不可写，才允许回退到用户提供路径或 `workspace/<skill-slug>-data/`。
- 执行结果必须打印 `output_dir=...` 便于核对真实落盘位置。
- 直接打开优化：一键全量产物生成后，自动复制一份快捷副本到 `~/clawdhome_shared/public/` 根目录（`quick_*` 字段）。
...
- 在第一次响应里先输出“环境检查结果”（一段汇总即可），说明内置能力可直接使用：
  ...
  - 可选增强安装：`edge-tts` / `openai-whisper` / `ffmpeg`
- 若无法确认依赖状态，必须提示用户先执行：`bash scripts/doctor.sh --strict`
- 在 macOS 下，若缺少 `ffmpeg`，优先提示一键引导：`bash scripts/bootstrap_macos.sh`
...
5.2 文本输入默认生成两份音频：
   ...
   - 原文全文音频：`<prefix>.full.mp3`（基于 `.full.txt`）
...
8. 跨平台（Linux/macOS/Windows）优先使用 `scripts/audio_bridge.py`：
   ...
   - 免费高质量 TTS 依赖：`python3 -m pip install edge-tts`
...
11. 执行命令（推荐）：
   - 一键全量产出（推荐）：`python3 scripts/generate_meeting_bundle.py --input <minutes-text-file> --topic <meeting-topic>`
   - 该命令强制校验 6 项产物：`txt + spoken.txt + audio.mp3 + audio_full.mp3 + mindmap.html + mindmap.xmind`。
   - 任一缺失即失败，禁止返回“已完成”。
12. 执行命令（仅语音）：
